...
class Attack:
    def __init__(self, leakage_model, correct_key, nb_traces_attacks=2000, nb_attacks=5, attack_byte=2,
    ...
    def perform_attacks(self, predictions, plt_attack):
        ...
        for i in range(self.nb_attacks):
            ...
            #     if results[:, self.correct_key][j] == 0:
            #         na = j
            #     else:
            #         break
            # print(na)
    ...
    def rank_compute(self, prediction, att_plt):
        ...
        key_log_prob_summary = np.zeros((nb_traces, 256))
        ...
        for i in range(nb_traces):
            if self.leakage_model == 'ID':
                predicted_output = prediction[
                    i, Utility.AES_Sbox()[np.bitwise_xor(range(256), int(att_plt[i, self.attack_byte]))]]
            else:
                predicted_output = prediction[i, Utility.calculate_HW(
                    Utility.AES_Sbox()[np.bitwise_xor(range(256), int(att_plt[i, self.attack_byte]))])]
            key_log_prob_summary[i] = predicted_output
